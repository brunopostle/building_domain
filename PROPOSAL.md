...
class ProcessRelation(ProvenanceMixin):
    ...
    subject_id: str | None = None  # Entity UUID whose Pass 5 call produced this edge
...
**`subject_id` (context scoping, `building_domain-eue`):** Pass 5 asks its ordering question from the viewpoint of one subject entity at a time ("what comes before/after `{entity_name}`") — every edge it writes is inherently scoped to that call. `subject_id` records that entity's UUID (nullable — `NULL` means "universal", i.e. a context-free rule that MUST hold everywhere; used for rows predating this field and for genuinely universal edges). This exists because generic canonical activities (e.g. "Concrete Curing", "Foundation Construction") are single shared `Entity` rows referenced from many unrelated subject contexts (different components, systems, building types); without recording which context asserted an edge, cycle detection (Section 10.4) cannot tell a real contradiction from two unrelated contexts' locally-true-but-globally-incompatible orderings colliding on the same shared node. Pass 5 MUST set `subject_id` to the entity it is currently processing when writing a new row. Rows written before this field existed are backfilled by parsing the subject entity's name out of `source_prompt` (Pass 5's prompt template always opens with it) and resolving it via the same name/alias lookup Pass 5 itself uses — exposed as `bsos curate backfill-process-context` (Section 17, curate spec).
...
## 10.4 Process Graph Cycle Detection (`building_domain-eue`)

Unlike Sections 10.1/10.2, a cycle is N-shaped (an arbitrary-size strongly connected component), not pair-shaped, so it is not recorded in `conflict_pairs`. The conflict detection batch computes this as a distinct sub-task:

* Load all `ProcessRelation` rows with `status IN ('proposed', 'accepted')` and build a directed graph, edges `predecessor_id → successor_id`.
* **Partition by `subject_id` before computing strongly connected components** — do NOT union every edge into one global graph. Build one subgraph per distinct non-`NULL` `subject_id` value (that context's own edges plus every `subject_id IS NULL` "universal" edge, which is included in every partition since a context-free rule must hold everywhere), plus one subgraph of the universal edges alone. Run `nx.strongly_connected_components` independently per partition. This is what prevents two unrelated contexts that each assert a locally-true ordering through a shared generic node (e.g. context A: "Waterproofing → Concrete Curing", context B: "Concrete Curing → Waterproofing") from being unioned into a false global cycle — they only land in the same partition, and thus the same cycle check, when they actually share a `subject_id` or are both universal.
* For each strongly connected component of size ≥ 2 found in any partition, mark every `ProcessRelation` edge with both endpoints in that component as `status = 'conflicted'`. Deduplicate identical edge-sets seen from multiple partitions (a purely-universal cycle reappears in every per-context partition since universal edges are always included) before reporting a count.
* **Durable resolution:** before marking a component's edges conflicted, compute a deterministic identity for it — `sha256("|".join(sorted(edge_ids)))` — and check for an existing `ReviewDecision` with `item_type = "process_relation_cycle"` and `item_id` equal to that hash and `decision = "keep-all"`. If found, skip re-flagging (leave the edges at their current, human-reviewed status) instead of unconditionally flipping them back to `conflicted`. This is required because a human "not actually a conflict, keep all edges" decision (`review-pending`, Section 17) only changes row `status`, not the graph shape — without this check, the exact same strongly connected component would be re-derived and the edges immediately re-flagged conflicted on the very next `bsos validate --conflicts` run. If the edge set changes at all (an edge added, removed, or deprecated), the hash changes and the new shape is treated as an unreviewed cycle — mirroring the "recompute whether outstanding work still exists rather than trusting a stored boolean" principle used for Pass 10 resume-safety (`building_domain-5ut`).
* This detects extraction-time cycles (Pass 5 independently extracted "X precedes Y" from one context and "Y precedes X" from another) before they are encountered at query time by `get_process_sequence` (Section 13.3).
* Reviewed via `bsos review pending --type process_relation` (Section 17), which groups cycle-conflicted rows (found as `status = 'conflicted'` AND absent from `conflict_pairs` — the complement of the pair-shaped conflicts Section 10.1/10.2 produce) by connected component so a reviewer sees the whole cycle at once, and offers `break=<indices>` (deprecate the offending edge(s), accept the rest), `keep-all` (writes the durable `process_relation_cycle` decision above), `deprecate-all`, or `defer`.
...
* `bsos validate --conflicts [--limit N]` — runs the conflict detection batch (Sections 10.1, 10.2, 10.4): (1) processes all `proposed` assertion/constraint/pattern items where `conflict_evaluated_at IS NULL`, runs LLM classification for each candidate pair above the embedding similarity threshold, writes a row to `conflict_pairs` for each `contradictory` pair, updates item status (`conflicted`, `accepted`, or remains `proposed`), and sets `conflict_evaluated_at = now()` on each evaluated item; (2) checks all `ProcessRelation` pairs for `hard_constraint` boolean divergence across models (Section 10.2) and marks divergent pairs as `conflicted`, writing corresponding rows to `conflict_pairs`; (3) checks for directed cycles in the ProcessRelation graph, computed per `subject_id` partition rather than one global graph (Section 10.4) — for each strongly connected component of size ≥ 2 not already covered by a durable `keep-all` decision, all ProcessRelation edges in the cycle are marked `status = "conflicted"` and added to the review queue with a note identifying the cycle members. This detects extraction-time cycles (where Pass 5 independently extracted "X precedes Y" and "Y precedes X") before they are encountered at query time, while the per-context partitioning avoids flagging unrelated contexts' independently-true orderings as a false contradiction merely because they share a generic node. Outputs a summary: items evaluated, conflicts found, items auto-promoted, ProcessRelation divergences flagged, process graph cycles detected, cyclic edges marked, and cycles already resolved (skipped due to a prior durable `keep-all` decision). `--limit N` stops after N LLM classification calls for that invocation (default: unlimited); items above the limit retain `conflict_evaluated_at IS NULL` and are processed on the next invocation. Use `--limit` to control API costs on large corpora — a corpus of 5,000 assertions can produce O(N²) candidate pairs before embedding pre-filtering reduces the count.
...
* `bsos curate backfill-process-context` — populates `ProcessRelation.subject_id` (Section 4.6, `building_domain-eue`) for rows written before that field existed, by parsing the subject entity's name out of each row's `source_prompt` (Pass 5's prompt template always opens with it) and resolving it through the same name/alias lookup Pass 5 uses. Rows with no parseable/resolvable subject are left `NULL` — cycle detection (Section 10.4) already treats `NULL` as "universal", so this is a safe default rather than an error. Reports counts updated/skipped.
